...
- Front-end plugin failure reporting: when the web is UP but a client plugin fails (render-time entry crashes are reported precisely via `slots.onEntryError`; module-load failures only when the browser gives an explicit dynamic-import failure message — window `error` events are NOT listened to, so ordinary page script errors are never mistaken for plugin failures); the watchdog runs a "degraded repair" (skips live dirs in the snapshot, relaunches web to reload plugins afterwards; if the agent changed nothing it clears the report without relaunching).

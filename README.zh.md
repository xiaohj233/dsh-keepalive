...
- 修复进度页：web 启动失败进入修复时，看门狗在 web 端口（默认 3080）上提供零依赖状态页——显示"启动发生错误，正在修复"、修复代理实时进度与已用时间；修复成功后页面自动跳回 web 界面，失败则显示修复代理的结论。
- 修复代理可对【本次报错插件所属的 link 目标开发目录】内的源码做最小修复（如补变量声明、修正 import、修复语法），该目录被内容快照覆盖、失败回滚；禁止删除文件。
- 修复代理的工作区根自动取所有用户 link 目标开发目录的公共父目录（如 `D:\Code`），官方 `@deepseek-ai/*` 的 link 包被排除、不可写；沙箱写入由快照审计兜底。
- 前端插件失败上报：web 存活但某个 client 插件模块加载失败时上报（仅限浏览器对动态 `import()` 失败的明确包装消息，如 "Failed to fetch dynamically imported module"；**不监听 window error、不使用 slots.onEntryError**——普通脚本错误与 entry 渲染崩溃（如 React #130）都不是插件模块故障，误报只会骚扰正常使用），看门狗触发"降级修复"（跳过 live 目录快照，修复后自动重启 web 重载插件；若修复代理未做任何改动则不重启，直接清除上报）。
- 全局修复进度浮层：任何页面右下角实时显示"正在修复：阶段 + agent 进度行"；仅在 `status=repairing` 时展示，修复结束自动消失，失败显示原因。

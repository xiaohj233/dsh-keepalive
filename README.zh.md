...
- 前端插件失败上报：web 存活但某个 client 插件失败时（渲染期 entry 崩溃经 `slots.onEntryError` 精确上报；模块加载失败仅当浏览器给出明确的动态 import 失败消息时上报——**不监听 window error**，避免把普通页面脚本错误误报成插件故障），看门狗触发"降级修复"（跳过 live 目录快照，修复后自动重启 web 重载插件；若修复代理未做任何改动则不重启，直接清除上报）。

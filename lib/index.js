...
const RESTARTER = fileURLToPath(new URL("../restarter.mjs", import.meta.url));
...
/** Cooldown between accepted front-end plugin-failure reports (per process). */
const PLUGIN_FAILURE_COOLDOWN_MS = 5 * 60 * 1000;
let lastPluginFailureAt = 0;

/** Record a front-end plugin load failure so the watchdog can repair it even
 * while the web process stays up. Throttled so a misbehaving plugin cannot
 * make the watchdog hot-loop. */
function reportPluginFailure({ plugin, error }) {
	const now = Date.now();
	if (now - lastPluginFailureAt < PLUGIN_FAILURE_COOLDOWN_MS) return false;
	lastPluginFailureAt = now;
	try {
		writeState({ pluginFailure: { plugin, error, at: new Date().toISOString() } });
		ctxLogger?.info(`[dsh-keepalive] plugin failure reported: ${plugin || "(unknown)"} — ${String(error).slice(0, 120)}`);
		return true;
	} catch (error) {
		ctxLogger?.warn(`[dsh-keepalive] plugin failure report failed: ${String(error)}`);
		return false;
	}
}
...
/** Delay before the fallback restarter relaunches the web itself: longer
 * than any configured check interval, so a live watchdog always wins the
 * relaunch race and the helper only fires when no watchdog is around. */
function restartFallbackDelayMs(checkIntervalMs) {
	return Math.max(10000, (Number(checkIntervalMs) || 5000) + 5000);
}

/** Schedule a manual web restart: record the request, spawn the detached
 * fallback restarter, then exit this process so the watchdog (when alive)
 * relaunches the web with the same binary and arguments. */
function scheduleManualRestart() {
	const state = readState();
	const bin = typeof state.dshBin === "string" && state.dshBin.length > 0
		? state.dshBin
		: typeof process.argv[1] === "string" ? process.argv[1] : "";
	if (bin.length === 0) return { ok: false, error: "cannot locate dsh bin.js" };
	const spec = JSON.stringify({
		bin,
		args: process.argv.slice(1),
		stateFile: STATE_FILE,
		intervalMs: Number(state.checkIntervalMs) || 5000
	});
	try {
		const child = spawn(process.execPath, [RESTARTER, spec], {
			detached: true,
			stdio: "ignore",
			windowsHide: true
		});
	...
	writeState({ manualRestart: { at: new Date().toISOString(), webPid: process.pid } });
	return { ok: true, watchdogAlive: watchdogAlive(state), delayMs: restartFallbackDelayMs(state.checkIntervalMs) };
}
...
function statusPayload() {
	...
	/* Live repair progress: the watchdog persists it to a sidecar file while
	 * a repair runs (startup-failure page OR degraded in-web repair), so the
	 * settings card can show "正在修复" even when the web owns the port. */
	let repairProgress = null;
	/* Only surface progress while a repair is actually running: a stale
	 * sidecar file (e.g. a crashed watchdog) must never render as
	 * "repairing" once the state has moved past `repairing`. */
	if (state.status === "repairing") {
		try {
			const pf = join(DSH_HOME, "keepalive-repair-progress.json");
			if (existsSync(pf)) {
				const parsed = JSON.parse(readFileSync(pf, "utf8"));
				if (Date.now() - Number(parsed.updatedAt) < 10 * 60 * 1000) repairProgress = parsed;
			}
		} catch {
			repairProgress = null;
		}
	...
	return {
		...
		lastManualRestartAt: state.manualRestart?.at ?? null,
...
function syncStateFromSection(section) {
	...
	writeState({
		...
		checkIntervalMs: Math.max(MIN_CHECK_INTERVAL_MS, Number(section.checkIntervalMs) || 5000),
		bootWaitMs: Math.max(MIN_BOOT_WAIT_MS, Number(section.bootWaitMs) || 25000)
	});
}
...
/** Maximum request body accepted by the config route (small settings payloads). */
...
/** Browser-trust fence mirroring dsh-client-connection's /api gate. */
...
/** Current LAN IPv4 literals, so the LAN page (Host header) is not rejected. */
...
/** Whether a request may reach a keepalive route (Host + Origin + fetch-site). */
function isTrustedApiRequest(req) {
	...
	const trusted = isLoopbackHostname(hostname) || lanAddresses().includes(hostname);
...
/** Minimums that keep the watchdog from hot-looping or killing early. */
const MIN_CHECK_INTERVAL_MS = 1000;
const MIN_BOOT_WAIT_MS = 5000;
...
function apply(ctx, config) {
	 ...
	 * on the watchdog already running. v2: every installed copy is decided
	 * independently and the engine never throws for installation-state
	 * reasons (version mismatch, unreadable manifest, drifted anchor all
	 * skip that copy only) — see lib/installed-patches.mjs. A failure here
	 * is logged, never fatal: the host keeps running, settings registration
	 * below proceeds untouched, and the card simply cannot save until the
	 * patch applies (e.g. after a DSH upgrade the guard refuses a changed
	 * package version instead of corrupting it).
	 *
	...
	if (argvBin !== void 0 && argvBin.endsWith("bin.js")) {
		...
		if (located.ok) {
			try {
				...
				if (!applied.ok) {
					ctx.logger?.warn(`[dsh-keepalive] installed-package patches incomplete: ${applied.summary}`);
				}
			} catch (error) {
				/* belt-and-braces: even an unexpected patch failure must not
				 * stop the host from registering its settings below. */
				ctx.logger?.warn(`[dsh-keepalive] installed-package patches failed: ${error instanceof Error ? error.message : String(error)}`);
			}
		} else {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			const scheduled = scheduleManualRestart();
			if (!scheduled.ok) {
				json(res, 500, { ok: false, error: scheduled.error });
				return;
			}
			ctx.logger?.info(`[dsh-keepalive] manual restart requested (watchdog ${scheduled.watchdogAlive ? "alive" : "absent"}, fallback delay ${scheduled.delayMs}ms)`);
			json(res, 200, { ok: true, restarting: true, watchdogAlive: scheduled.watchdogAlive });
			/* Let the response flush, then exit: the watchdog relaunches the
			 * web, or the fallback restarter does when no watchdog is up. */
			setTimeout(() => process.exit(0), 400);
		}
	}), "dsh-keepalive: /api/keepalive/restart");
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			if (typeof body.checkIntervalMs === "number") {
				next.checkIntervalMs = Math.max(MIN_CHECK_INTERVAL_MS, Math.trunc(body.checkIntervalMs));
			}
			if (typeof body.bootWaitMs === "number") {
				next.bootWaitMs = Math.max(MIN_BOOT_WAIT_MS, Math.trunc(body.bootWaitMs));
			}
	...
	ctx.effect(() => ctx.webServer.register({
		kind: "exact",
		path: "/api/keepalive/report-plugin-failure",
		handler: async (req, res) => {
			if (!isTrustedApiRequest(req)) {
				res.writeHead(403);
				res.end("forbidden");
			...
			const body = await readJsonBody(req);
			if (body === null || typeof body.error !== "string" || body.error.trim().length === 0) {
				res.writeHead(400);
				res.end("bad request");
				return;
			}
			const plugin = typeof body.plugin === "string" ? body.plugin.slice(0, 200) : "";
			const accepted = reportPluginFailure({ plugin, error: body.error.slice(0, 2000) });
			json(res, 200, { ok: accepted });
		}
	}), "dsh-keepalive: /api/keepalive/report-plugin-failure");

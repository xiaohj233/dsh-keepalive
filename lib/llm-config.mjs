...
export function parseKeepaliveConfig(text) {
	try {
		...
		return {
			...
			repairReasoningEffort: get("repairReasoningEffort", "max")
		};
	} catch {
		return null;
	}
...
export function writeKeepaliveConfig(text, patch) {
	const section = [
		...
		`  repairReasoningEffort: ${yamlString(patch.repairReasoningEffort ?? "max")}`
	].join("\n") + "\n";
...
export function renderRepairRuntimeSettings(opts) {
	const effort = opts.reasoningEffort ?? "max";
	return [
		"permission:",
		...
		`  reasoningEffort: ${yamlString(effort)}`,
		"llm-pi-ai:",
		"  providers:",
		"    repair:",
		...
		"            high: high",
		"            max: max",
		`          defaultReasoningEffort: ${yamlString(effort)}`
	].join("\n");
}
